...
- [x] T3.2 分类、标签、资源、来源、授权记录/证据、编辑复审和审核状态机
- [x] T3.3 夸克/百度/通用外链适配器、URL 安全、链接模型与检查策略
- [x] T3.4 outbox、幂等 Worker、Meilisearch 增量索引与 `swap-indexes` 原子重建
- [x] T3.5 搜索首页、结果、详情、分类/网盘/权利/链接筛选、排序、建议和本机历史
- [x] T3.6 安全跳转、渠道模板、点击事件、五分钟去重和推广披露
- [x] T3.7 链接检查 Worker、确定性失败双确认、退避、死信和检查历史
- [x] T3.8 提交、举报、侵权通知、恢复申请、黑名单和公开票据
- [x] T3.9 CSV/Markdown 预览确认导入、幂等和逐行报告
- [x] T3.10 报表、原始事件保留清理、加密备份恢复；真实转化仅保留模型，未获官方接口前不启用
...
- [~] T4.4 SSRF、开放重定向、CSRF、XSS、越权、限流、敏感泄露测试（URL/模板/CSRF/限流单测已覆盖，完整 API 加固待补）
- [~] T4.5 Worker mock、重试/死信/幂等和外部平台隔离测试（状态双确认已覆盖，外部请求 mock 与死信集成待补）
- [ ] T4.6 Playwright 核心流程、移动布局和基本无障碍检查
- [~] T4.7 恶意 CSV/Markdown、部分失败、备份恢复演练（解析攻击与本地加密恢复已验证，批量部分失败 API 集成待补）
