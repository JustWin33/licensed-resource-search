import { counterNoticeSchema, createCounterNotice } from '@web/src/server/governance-service';
...
export async function POST(request: Request) {
  ...
  try {
    await enforceRateLimit(request, { scope: 'counter-notice', limit: 5, windowSeconds: 3600 });
    await requirePublicCsrf(request);
    const result = await createCounterNotice(counterNoticeSchema.parse(await request.json()));
