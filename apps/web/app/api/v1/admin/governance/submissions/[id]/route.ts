...
export async function PATCH(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const input = updateSubmissionStatusSchema.parse(await request.json());
    const result = await updateSubmissionStatus((await context.params).id, input.status, actor, id);
