import { createTakedown, takedownSchema } from '@web/src/server/governance-service';
...
export async function POST(request: Request) {
  ...
  try {
    await enforceRateLimit(request, { scope: 'takedown', limit: 5, windowSeconds: 3600 });
    await requirePublicCsrf(request);
    const result = await createTakedown(takedownSchema.parse(await request.json()));
